    ...
    if ($p_name != "" && $p_price != "" && $p_type != "" && $p_active != "") {
        ...
        if (isset($_FILES['file']['name']) && $_FILES['file']['name'] != "") {
            ...
            if ($_FILES['file']['type'] == 'image/jpeg' || $_FILES['file']['type'] == 'image/png') {
                ...
                $sql="INSERT INTO `product` (`Name`, `Price`,`Type`, `Active`,Img) VALUES ('$p_name','$p_price','$p_type', '$p_active','$img')";
 
                if (execute_sql($conn, $dbname, $sql)) {
                    echo ('{"state":true,"message":"商品新增成功","datainfo":'.json_encode($datainfo).'}');
                } else {
